...
export function buildModelStrategyBrief(reading) {
  ...
  return `【解读策略 · 模型主通道】
你不是填模板，而是塔罗师：先逐张读本次牌面，再三张合参定调，产出你的主观观点；每个判断都要能指回具体牌名/正逆位/意象，禁止用规则表或单张重牌直接定结论。
...
${chain || "（见下方牌面输入）"}
- 现状：自我此刻如何呈现
- 阻碍：中间变量位——按该牌正逆位与关键词解读；牌位名不等于凶兆
- 建议：可被意识承接的态度；配本周一个具体动作；建议张对总倾向有同等权重
- 总判断：三张合参后定倾向；观点明确，必须与本次牌面一致，禁止单张重牌压过另外两张
